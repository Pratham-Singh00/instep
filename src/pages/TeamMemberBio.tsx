...
// import { useContent } from "@/context/ContentContext"; // Removed unused import
...
const TeamMemberBio = () => {
  ...
  const prevMember = memberIndex > 0 ? teamMembers[memberIndex - 1] : null;
  const nextMember = memberIndex < teamMembers.length - 1 ? teamMembers[memberIndex + 1] : null;

  return (
    <div className="min-h-screen">
      <SEO
        title={`${member.name} - ${member.qualification} | InStep PC Team`}
        description={`Meet ${member.name}, ${member.qualification} at InStep PC. ${member.bio.substring(0, 150)}...`}
        image={member.image}
        url={`/team/${member.id}`}
        type="profile"
      />
